...
export default function GalleryCard({ gallery, setOpenDialog, isEdit = true, setSelectedId }) {
  ...
  return (
    <Card className="group !rounded-lg">
      ...
      <Stack spacing={1} sx={{ py: 1, px: 2, bgcolor: 'white' }}>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" className="!mt-0">
          <Typography variant="caption">{gallery?.mission}</Typography>
        </Stack>
      </Stack>
        ...
        {isEdit ? (
          <Tooltip title="Edit">
            <Iconify
              ...
              onClick={() => navigate(`/gallery/add?galleryId=${gallery?._id}`)}
